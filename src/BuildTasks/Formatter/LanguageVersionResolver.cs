...
internal static class LanguageVersionResolver
{
    ...
    public static LanguageVersionResolution Resolve(string configured, string projectPath)
    {
        string requested = configured.Trim();

        if (requested.Length == 0)
            return new(LanguageVersion.Default, Substituted: false);

        if (LanguageVersionFacts.TryParse(requested, out LanguageVersion parsed))
            return new(parsed, Substituted: false);
        ...
        return IsNewerThanSupported(requested)
            ? new(LanguageVersion.Preview, Substituted: true)
    ...
    internal sealed record LanguageVersionResolution(LanguageVersion Version, bool Substituted);
}
